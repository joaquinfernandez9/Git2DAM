...
public class ReaderServImpl implements ReaderServ {
    ...
    @Override public Either<String, List<Reader>> getAll() {
        Either<String, List<Reader>> respuesta;
        if (daoReaderImpl.getAll().isRight()) {
            respuesta = Either.right(daoReaderImpl.getAll().get());
        } else {
            respuesta = Either.left(ServConstants.ERROR);
        }
        return respuesta;
    }
    ...
    @Override public void deleteReader(int idReader) {
